...
pub async fn run_with_ui(
    ...
    let _input_handle = input::spawn_listener(input_tx, shutdown.clone(), shared_hotkeys.clone())
    ...
    info!(
        profiles = initial_cfg.profiles.len(),
        "Ready — hold a mode hotkey to dictate."
    );
...
fn resolve_profile(
    ...
    if let Some(app_context) = app_context {
        if let Some(bound) = config.app_bindings.get(&app_context.id) {
            if let Some(profile) = config
                .profiles
                .iter()
                .find(|profile| &profile.name == bound)
            {
...
fn hotkey_signature(config: &ConfigV2) -> Vec<(String, String)> {
    ...
    signature.push((
        "__hands_free__".into(),
        config.global.hands_free_hotkey.clone(),
    ));
    signature.push((
        "__voice_edit__".into(),
        config.global.voice_edit_hotkey.clone(),
    ));
    signature
}
...
async fn state_recording(
    ...
    let audio_thread_handle = match audio::start_capture(
        ...
        {
            warn!(device=configured_device.as_deref().unwrap_or_default(),%first_error,"Configured microphone unavailable; falling back to system default");
            match audio::start_capture(audio_tx.clone(), recording_flag.clone(), None) {
                Ok(handle) => handle,
                Err(error) => {
                    error!(%error,"Failed to start fallback audio capture");
    ...
    drop(audio_tx);
    let first_chunk =
        match audio_rx.recv_timeout(std::time::Duration::from_millis(AUDIO_STARTUP_TIMEOUT_MS)) {
            Ok(chunk) => chunk,
            Err(error) => {
    ...
    let collector_handle = tokio::task::spawn_blocking(move || {
        let mut samples = Vec::<i16>::with_capacity(480_000);
    ...
    let selected_text = if operation == Operation::VoiceEdit {
        tokio::time::sleep(std::time::Duration::from_millis(
            VOICE_EDIT_RELEASE_SETTLE_MS,
        ))
        .await;
        match tokio::task::spawn_blocking(selection::capture_selected_text).await {
            Ok(Ok(Some(text))) => Some(text),
            Ok(Ok(None)) => {
                warn!("Voice Edit completed without selected text");
                if config.global.sound_enabled {
                    crate::audio_feedback::play_error_beep();
                }
                return;
    ...
    let recovery_id = match recovery::persist_with_context(
        ...
        Err(error) => {
            error!(%error,"Could not persist recovery audio; continuing");
            None
        }
    };
    ...
    if final_text.trim().is_empty() {
        preserve_failure(
            ...
            "Post-processing produced empty text",
        );
    ...
    match crate::tracking::append_record(&record) {
        ...
        Err(error) => {
            preserve_failure(
                recovery_id.as_deref(),
                &format!("Tracking save failed: {error}"),
            );
            warn!(%error,"Failed to save tracking record; recovery audio kept");
        }
    }
    if operation == Operation::VoiceEdit {
        let current_context = tokio::task::spawn_blocking(context::capture)
            .await
            .ok()
            .flatten();
...
mod tests {
    ...
    fn explicit_mode_wins() {
        ...
        config
            .app_bindings
            .insert("web:chrome:gmail".into(), "email".into());
        assert_eq!(
            resolve_profile(&config, "email", Some(&gmail()))
                .unwrap()
                .name,
            "email"
        );
    ...
    fn default_resolves_binding() {
        ...
        config
            ...
            .insert("web:chrome:gmail".into(), "email".into());
        assert_eq!(
            resolve_profile(&config, "dictation", Some(&gmail()))
                .unwrap()
                .name,
            "email"
        );
    ...
    #[test]
    fn context_change_blocks_edit() {
        let a = context::AppContext {
            id: "app:code".into(),
            app_name: "Code".into(),
