...
pub fn list() -> Result<Vec<RecoveryItem>> {
        ...
        .with_context(|| format!("Cannot read recovery directory {}", dir.display()))?
    {
        ...
        match fs::read_to_string(&path)
            .ok()
            .and_then(|raw| serde_json::from_str::<RecoveryItem>(&raw).ok())
        {
            Some(item) if wav_path(&item.id)?.exists() => items.push(item),
            Some(_) => {
...
pub fn remove(id: &str) -> Result<()> {
    ...
    if wav.exists() {
        fs::remove_file(&wav)
            .with_context(|| format!("Cannot remove recovery WAV {}", wav.display()))?;
    }
    if meta.exists() {
        fs::remove_file(&meta)
            .with_context(|| format!("Cannot remove recovery metadata {}", meta.display()))?;
    }
    Ok(())
}
...
fn decode_pcm16_mono_wav(bytes: &[u8]) -> Result<Vec<i16>> {
    ...
    while offset + 8 <= bytes.len() {
        ...
        if end > bytes.len() {
            bail!("Corrupted recovery WAV chunk");
        }
        if id == b"data" {
