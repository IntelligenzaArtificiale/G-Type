...
pub async fn transcribe(config: &ConfigV2, samples: &[i16]) -> Result<(String, TokenUsage)> {
    if samples.is_empty() {
    ...
    let p0 = config.profiles.get(0).expect("At least 1 profile must exist");
    let model_name = p0.model.strip_prefix("models/").unwrap_or(&p0.model);
    let url = format!(
        "https://generativelanguage.googleapis.com/v1beta/models/{}:generateContent",
        model_name
    );
    let body = build_request_body(&wav_b64, &config.global.language);

    debug!(model = %p0.model, "Sending request to Gemini API");

    // Step 3: Send HTTP POST (with auto-retry for 429/503)
    // API key is sent via header, never in URL (security best practice).
    
    let api_key = config.keys.get("gemini").context("No API key available for gemini")?;
