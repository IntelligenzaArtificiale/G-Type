...
pub struct SharedHotkeys {
    bindings: RwLock<HotkeySet>,
}

impl SharedHotkeys {
    pub fn new(bindings: HotkeySet) -> Arc<Self> {
        Arc::new(Self {
...
pub fn parse_hotkey(raw: &str) -> Result<Hotkey> {
    ...
    Ok(Hotkey {
        modifiers,
        trigger: trigger.context("Missing trigger key")?,
        label: raw.to_string(),
    })
}
...
impl HookState {
    ...
    fn matches(&self, hotkey: &Hotkey) -> bool {
        hotkey
            .modifiers
            .iter()
            .all(|modifier| self.modifiers.contains(modifier))
            && self.triggers.contains(&hotkey.trigger)
    }
    ...
    fn handle(&mut self, event: &Event) {
        match event.event_type {
            EventType::KeyPress(key) => {
                let changed = if let Some(modifier) = key_to_modifier(key) {
    ...
    fn check_combo(&mut self) {
        ...
        if bindings
            .hands_free
            .as_ref()
            .is_some_and(|hotkey| self.matches(hotkey))
        {
            if self.debounced() {
                return;
            }
        ...
        for item in &bindings.profiles {
            if self.matches(&item.0)
                && best
                    .as_ref()
                    .map(|candidate| item.0.modifiers.len() > candidate.0.modifiers.len())
                    .unwrap_or(true)
            {
                best = Some(item);
            }
    ...
    fn check_release(&mut self) {
        let Some(active) = self.active_hold.clone() else {
            return;
        };
        let bindings = self.shared.read().clone();
        let held = match &active {
            HoldAction::Profile(name) => bindings
                .profiles
                .iter()
                .find(|(_, profile_name)| profile_name == name)
                .is_some_and(|(hotkey, _)| self.matches(hotkey)),
            HoldAction::VoiceEdit => bindings
                .voice_edit
                .as_ref()
                .is_some_and(|hotkey| self.matches(hotkey)),
        };
        ...
        let signal = match active {
            HoldAction::Profile(_) => InputSignal::StopProfile,
            HoldAction::VoiceEdit => InputSignal::StopVoiceEdit,
        };
        let _ = self.tx.blocking_send(signal);
    }
}
...
pub fn spawn_listener(
    ...
    #[cfg(target_os = "linux")]
    if is_wayland() {
        return spawn_evdev_listener(tx, shutdown, shared);
    }
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            if let Err(error) = rdev::listen(callback) {
                error!(?error, "Global keyboard listener crashed");
            }
        })
...
fn spawn_evdev_listener(
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
                ...
                if let Ok(file) = std::fs::OpenOptions::new().read(true).open(&path) {
                    if set_nonblocking(&file).is_ok() {
                        ...
                        readers.push(std::thread::spawn(move || {
                            evdev_loop(file, state, shutdown)
                        }));
                    }
                }
            }
            ...
            if readers.is_empty() {
                warn!("Wayland evdev unavailable; falling back to rdev/XWayland");
                let state = state.clone();
                let callback = move |event: Event| {
                    if !shutdown.load(Ordering::Relaxed) {
                        if let Ok(mut state) = state.lock() {
...
fn evdev_loop(
    ...
    while !shutdown.load(Ordering::Relaxed) {
        ...
        let read = unsafe { libc::read(fd, event.as_mut_ptr().cast(), size) };
        if read == size as isize {
            let event = unsafe { event.assume_init() };
            if event.event_type == 1 && event.value != 2 {
                if let Some(key) = linux_keycode(event.code) {
...
fn find_keyboard_devices() -> Vec<std::path::PathBuf> {
    ...
    if let Ok(entries) = std::fs::read_dir("/dev/input/") {
        for entry in entries.flatten() {
            ...
            let caps = format!("/sys/class/input/{name}/device/capabilities/ev");
            if let Ok(value) = std::fs::read_to_string(caps) {
...
mod tests {
    ...
    fn parses_voice_controls() {
        ...
        assert_eq!(parse_hotkey("ctrl+shift+e").unwrap().trigger, Key::KeyE);
    }
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn linux_codes_cover_voice_controls() {
        assert_eq!(linux_keycode(35), Some(Key::KeyH));
        assert_eq!(linux_keycode(18), Some(Key::KeyE));
        assert_eq!(linux_keycode(57), Some(Key::Space));
    }
}
